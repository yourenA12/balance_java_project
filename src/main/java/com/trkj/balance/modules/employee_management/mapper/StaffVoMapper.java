...
public interface StaffVoMapper extends BaseMapper<StaffVo> {
   ...
   //分页查询员工状态为正式和试用的员工
    @Select("select * from staff s left JOIN dept d on s.DEPT_ID=d.DEPT_ID left JOIN  DEPT_POST dp on s.DEPT_POST_ID=dp.DEPT_POST_ID where s.STAFF_STATE=2 or s.STAFF_STATE=3")
 ...
 //查询员工状态为正式和试用的员工  查询员工的姓名、部门、名称
    ...
    //历史花名册 查询状态为离职的员工
    @Select("select * from staff s left JOIN dept d on s.DEPT_ID=d.DEPT_ID left JOIN  DEPT_POST dp on s.DEPT_POST_ID=dp.DEPT_POST_ID LEFT JOIN QUIT q on s.STAFF_ID=q.STAFF_ID where s.STAFF_STATE=1 and q.is_deleted=0")
